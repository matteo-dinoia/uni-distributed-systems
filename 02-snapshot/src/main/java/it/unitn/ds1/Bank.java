...
public class Bank extends AbstractActor {
    ...
    private final boolean snapshotInitiator;  // the node is a snapshot initiator
    ...
    private void sendTokens() {
        ...
        for (ActorRef p : peers) {
            // System.out.println("Bank " + id + " sending token to" + p.path().name());
            p.tell(t, getSelf());
        }
    }
    ...
    private void captureState() {
        System.out.println("Starting to capture the state. Bank " + id + " snapId: " + snapId
                + " state: " + balance);
        ...
        // TODO 0: Start considering inside which function and under which condition you should
        // print the balance for it to be consistent (consistent cut) NOTE: Print your state only
        // after the end of the snapshot protocol for this node
        ...
        //System.out.println("STATE FROM " + getSender().path().name() + " TO " + getSelf().path().name());
    }
    ...
    private void onJoinGroupMsg(JoinGroupMsg msg) {
        ...
        System.out.println(id + ": starting with " + msg.group.size() + " peer(s)");
    ...
    private void onToken(Token token) {
        ...
        //System.out.println("FROM " + getSender().path().name() + " TO " + getSelf().path().name());
        if(!this.stateBeingCaptured) {
            captureState();
            sendTokens();
        }
        ...
        if(tokensReceived.size() == peers.size()){
            System.out.println("Bank " + id + " snapId: "+ snapId + " state: " + (this.capturedBalance + this.moneyInTransit));
